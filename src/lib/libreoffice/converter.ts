 ...
 * 2. Requests uncompressed paths (soffice.wasm / soffice.data), while the asset
 *    loader can fall back to .gz or .gz chunks and decompress them in-browser.
...
// Request uncompressed names. Static builds can ship only .gz chunks; asset-loader
// reassembles and decompresses them before passing Blob URLs to the converter.
    ...
    private async checkEnvironment(): Promise<void> {
        ...
        await Promise.all(files.map(async (file) => {
            ...
            try {
                ...
                const candidates = [
                    url,
                    `${this.basePath}${file}.manifest.json?v=${ASSET_VERSION}`,
                    `${this.basePath}${file}.gz?v=${ASSET_VERSION}`,
                    `${this.basePath}${file}.gz.manifest.json?v=${ASSET_VERSION}`,
                ];

                let res: Response | null = null;
                let matchedUrl = url;
                for (const candidate of candidates) {
                    const candidateRes = await fetch(candidate, { method: 'HEAD' });
                    if (candidateRes.ok) {
                        res = candidateRes;
                ...
                if (res?.ok) {
                    ...
                    console.warn(
                        `[LibreOffice] ${file}: OK (${res.status}) ${duration}ms | ${sizeMb} | type=${type} | source=${matchedUrl}`
                    );
                } else {
                    console.error(`[LibreOffice] ${file}: FAILED`);
                    throw new Error(`Required file ${file} was not found`);
                }
            } catch (e) {
