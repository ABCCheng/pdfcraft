...
import { Search, X, Filter, Star, ShieldCheck, Zap, Wrench } from 'lucide-react';
...
export default function ToolsPageClient({ locale, localizedToolContent }: ToolsPageClientProps) {
  ...
  const featureHighlights = [
    ...
    },
    {
      ...
      description: t('home.features.powerful.description'),
    },
  ];
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[hsl(var(--color-background))]">
      ...
      <main className="flex-1">
        {/* Page Header */}
        <section className="relative pt-24 pb-10 border-b border-[hsl(var(--color-border))] bg-[hsl(var(--color-muted)/0.25)]">
          <div className="app-container relative z-10">
            <div className="max-w-3xl mx-auto text-center">
              <h1 className="text-3xl md:text-4xl font-bold text-[hsl(var(--color-foreground))] mb-4">
                {t('toolsPage.title')}
              </h1>
              <p className="text-base md:text-lg text-[hsl(var(--color-muted-foreground))] mb-7 leading-relaxed">
              ...
              <div className="relative max-w-2xl mx-auto">
                <div className="relative group">
                  <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-[hsl(var(--color-muted-foreground))] group-focus-within:text-[hsl(var(--color-primary))] transition-colors z-10" aria-hidden="true" />
                  <input
                    type="search"
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    placeholder={t('tools.search.placeholder')}
                    className="w-full pl-12 pr-4 py-3 text-base rounded-lg border border-[hsl(var(--color-border))] bg-[hsl(var(--color-card))] text-[hsl(var(--color-foreground))] placeholder:text-[hsl(var(--color-muted-foreground))] focus:outline-none focus:ring-2 focus:ring-[hsl(var(--color-primary)/0.2)] focus:border-[hsl(var(--color-primary))] transition-all"
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-7 text-left">
                {featureHighlights.map((feature) => {
                  ...
                  return (
                    <div key={feature.title} className="rounded-lg border border-[hsl(var(--color-border))] bg-[hsl(var(--color-background))] p-4">
                      <div className="flex items-center gap-2 mb-2">
                        <Icon className="h-4 w-4 text-[hsl(var(--color-primary))]" aria-hidden="true" />
                        <h2 className="text-sm font-semibold text-[hsl(var(--color-primary))]">
                          {feature.title}
                        </h2>
                      </div>
        ...
        {/* Filters and Tools */}
        <section className="py-8 bg-[hsl(var(--color-muted)/0.2)] min-h-[500px]">
          <div className="app-container">
            {/* Filter Bar */}
            <div className="flex flex-col md:flex-row items-center gap-4 mb-8 sticky top-16 z-40 py-3 px-4 rounded-lg border border-[hsl(var(--color-border))] bg-[hsl(var(--color-background))]/95 backdrop-blur transition-all">
